...
string base64_util(int a, int b, int c)
{
    ...
    string ABC = A+B+C;
    string x="", y="", z="", w="";
    for(int j=0; j<6; j++) x += ABC[j];
    for(int j=6; j<12; j++) y += ABC[j];
    for(int j=12; j<18; j++) z += ABC[j];
    for(int j=18; j<24; j++) w += ABC[j];
...
int main()
{
    ...
    cout<<"\nInput your string which you want to encode: \n";
    string s;
    getline(cin,s);
